...
La version retenue est la CMP 2.6.5, compilée par =RpR= et annoncée comme
...
- aucune archive CMP n'est publiée dans le dépôt ou les livrables du Heritage
  Pack ;
- l'utilisateur choisit la case CMP, puis l'installateur télécharge directement
  le commit épinglé depuis GitHub ;
- l'installation est interrompue si la taille, le SHA-256, le nombre d'entrées,
  les dossiers autorisés ou la liste multijoueur ne correspondent pas ;
