...
{
    internal static class Czech5OpelEffectInstaller
    {
        private const string RegistryPath = "Missions/CZECH5/Scripts.dta";
        private const string Actor = "La_OpelE_1";
        private const string Script = "Opel_1_OnDeath.scr";
        ...
        public static string ValidateOnly(string gamePath)
        {
            ...
            if (!BytesEqual(patched, PatchRegistry(patched)))
                throw new InvalidDataException(
                    "La restauration de l'effet de l'Opel Czech 5 n'est pas idempotente.");
            ValidateAssets(gamePath);
            return "Vestige Czech 5 verifie : l'OpelE_1 retrouve son effet "
                + "officiel a la destruction.";
        }
            ...
            Action<string> progress)
        {
            InstallerCore.Report(progress,
                "Reactivation de l'effet de destruction de l'Opel dans Czech 5...");
            DormantSource source = ResolveSource(
                gamePath, RegistryPath, MissionArchives);
            string target = InstallerCore.SafeGameTarget(gamePath, RegistryPath);
            ...
            {
                InstallerCore.Report(progress,
                    "Effet de l'Opel Czech 5 deja actif; aucune modification.");
                return;
            }
            ...
            InstallerCore.Log(
                "Czech 5 : La_OpelE_1 relie a Opel_1_OnDeath.scr.");
            InstallerCore.Report(progress,
                "L'effet de destruction officiel de l'Opel Czech 5 a ete reactive.");
        }
        ...
        private static byte[] PatchRegistry(byte[] data)
        {
            ...
            foreach (KeyValuePair<string, string> binding in bindings)
            {
                ...
                if (String.Equals(
                    ...
                    return data;
                throw new InvalidDataException(
                    "L'OpelE_1 de Czech 5 possede deja un autre script : "
                    + binding.Value + ".");
            }
        ...
        private static List<KeyValuePair<string, string>> ReadBindings(byte[] data)
        {
            if (data == null || data.Length < 6)
                throw new InvalidDataException(
                    "Registre de scripts Czech 5 trop court.");
            Encoding ansi = Encoding.GetEncoding(1252);
            ...
            byte[] data, ref int offset, Encoding encoding)
        {
            if (offset < 0 || offset > data.Length - 6
                ...
                throw new InvalidDataException(
                    "Champ invalide dans le registre Czech 5.");
            ...
            if (rawLength < 7 || rawLength > Int32.MaxValue
                ...
                throw new InvalidDataException(
                    "Taille de champ invalide dans le registre Czech 5.");
        ...
        private static void ValidateBinding(byte[] data)
        {
            ...
            if (found != 1)
                throw new InvalidDataException(
                    "La liaison de l'OpelE_1 Czech 5 n'est pas unique.");
        }

        private static void ValidateAssets(string gamePath)
        {
            ValidateAsset(gamePath,
                "Scripts/CZECH5/Opel_1_OnDeath.scr", ScriptArchives,
                new[] {
                    "FRM_GetMyFrame(me)",
                    "OnDeath()",
                    "FRM_CreateParticle(79, me)"
                });
            ValidateAsset(gamePath,
                "Scripts/CZECH5/CUTSCENE_cz4B.scr", ScriptArchives,
                new[] {
                    "FRM_FindFrame(ingame_car1,\"La_OpelE_1\")",
                    "FRM_SetOn(ingame_car1, true)"
                });
            ValidateAsset(gamePath,
                "Missions/CZECH5/actors.bin", MissionArchives,
                new[] { "La_OpelE_1", "La_OpelE_5" });

            DormantSource registry = ResolveSource(
                gamePath, RegistryPath, MissionArchives);
            List<KeyValuePair<string, string>> bindings =
                ReadBindings(ReadSource(registry));
            ValidateExistingBinding(bindings, "La_OpelE_5", "OpelOD.scr");
        }
            ...
            string actor, string script)
        {
            ...
            if (found != 1)
                throw new InvalidDataException(
                    "Liaison Czech 5 attendue absente : "
                    + actor + " -> " + script + ".");
        }
            ...
            string[] markers)
        {
            ...
            foreach (string marker in markers)
                if (text.IndexOf(marker, StringComparison.OrdinalIgnoreCase) < 0)
                    throw new InvalidDataException(
                        "Element Czech 5 manquant dans " + relative + ": "
                        + marker + ".");
        }
