...
{
    internal static class Alps2ShotAlarmInstaller
    {
        private const string RegistryPath = "Missions/ALPS2/Scripts.dta";
        private const string Actor = "shotalarmdetector";
        private const string Script = "shotalarmdetector.scr";
        ...
        public static string ValidateOnly(string gamePath)
        {
            ...
            if (!BytesEqual(patched, PatchRegistry(patched)))
                throw new InvalidDataException(
                    "La restauration du detecteur Alps 2 n'est pas idempotente.");
            ValidateAssets(gamePath);
            return "Vestige Alps 2 verifie : shotalarmdetector rejoint la chaine d'alarme officielle complete.";
        }
            ...
            Action<string> progress)
        {
            InstallerCore.Report(progress,
                "Reactivation du detecteur shotalarmdetector dans Alps 2...");
            DormantSource source = ResolveSource(
                gamePath, RegistryPath, MissionArchives);
            string target = InstallerCore.SafeGameTarget(gamePath, RegistryPath);
            ...
            {
                InstallerCore.Report(progress,
                    "Detecteur shotalarmdetector de Alps 2 deja active; aucune modification.");
                return;
            }
            ...
            InstallerCore.Log("Alps 2 : detecteur shotalarmdetector reactive.");
            InstallerCore.Report(progress,
                "Le detecteur shotalarmdetector d'Alps 2 a ete reactive.");
        }
        ...
        private static byte[] PatchRegistry(byte[] data)
        {
            ...
            foreach (KeyValuePair<string, string> binding in bindings)
            {
                ...
                if (String.Equals(
                    ...
                    return data;
                throw new InvalidDataException(
                    "Le detecteur shotalarmdetector possede deja un autre script : "
                    + binding.Value + ".");
            }
        ...
        private static List<KeyValuePair<string, string>> ReadBindings(byte[] data)
        {
            if (data == null || data.Length < 6)
                throw new InvalidDataException(
                    "Registre de scripts Alps 2 trop court.");
            Encoding ansi = Encoding.GetEncoding(1252);
            ...
            byte[] data, ref int offset, Encoding encoding)
        {
            if (offset < 0 || offset > data.Length - 6
                ...
                throw new InvalidDataException(
                    "Champ invalide dans le registre Alps 2.");
            ...
            if (rawLength < 7 || rawLength > Int32.MaxValue
                ...
                throw new InvalidDataException(
                    "Taille de champ invalide dans le registre Alps 2.");
        ...
        private static void ValidateBinding(byte[] data)
        {
            ...
            if (found != 1)
                throw new InvalidDataException(
                    "La liaison du detecteur shotalarmdetector dans Alps 2 n'est pas unique.");
        }

        private static void ValidateAssets(string gamePath)
        {
            ValidateAsset(gamePath,
                "Scripts/ALPS2/shotalarmdetector.scr", ScriptArchives,
                new[] {
                    "FRM_FindFrame(shotalarm,\"shotalarm\")",
                    "_PlayerInRange(4)",
                    "SendSignal (shotalarm,2)"
                });
            ValidateAsset(gamePath,
                "Scripts/ALPS2/shotalarm.scr", ScriptArchives,
                new[] { "Whenever sklad(_SignalReceived(2))" });
            ValidateAsset(gamePath,
                "Missions/ALPS2/scene2.bin", MissionArchives,
                new[] { "shotalarmdetector" });

            DormantSource registry = ResolveSource(
                gamePath, RegistryPath, MissionArchives);
            int alarmBindings = 0;
            foreach (KeyValuePair<string, string> binding
                in ReadBindings(ReadSource(registry)))
                if (String.Equals(binding.Key, "shotalarm",
                    StringComparison.OrdinalIgnoreCase)
                    && String.Equals(binding.Value, "shotalarm.scr",
                    StringComparison.OrdinalIgnoreCase))
                    alarmBindings++;
            if (alarmBindings != 1)
                throw new InvalidDataException(
                    "La chaine d'alarme officielle d'Alps 2 n'est pas complete.");
        }
            ...
            string[] markers)
        {
            ...
            foreach (string marker in markers)
                if (text.IndexOf(marker, StringComparison.OrdinalIgnoreCase) < 0)
                    throw new InvalidDataException(
                        "Element Alps 2 manquant dans " + relative + " : "
                        + marker + ".");
        }
