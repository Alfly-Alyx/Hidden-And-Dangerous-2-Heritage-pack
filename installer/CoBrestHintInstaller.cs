...
{
    internal static class CoBrestHintInstaller
    {
        private const string RegistryPath = "Missions/Co_brest/Scripts.dta";
        private const string Actor = "hint";
        private const string Script = "hint.scr";
        ...
        public static string ValidateOnly(string gamePath)
        {
            ...
            if (!BytesEqual(patched, PatchRegistry(patched)))
                throw new InvalidDataException(
                    "La restauration du conseil Brest coop n'est pas idempotente.");
            ValidateAssets(gamePath);
            return "Vestige Brest coop verifie : le conseil contextuel officiel 15600 retrouve sa liaison solo.";
        }
            ...
            Action<string> progress)
        {
            InstallerCore.Report(progress,
                "Reactivation du conseil contextuel dans Brest coop...");
            DormantSource source = ResolveSource(
                gamePath, RegistryPath, MissionArchives);
            string target = InstallerCore.SafeGameTarget(gamePath, RegistryPath);
            ...
            {
                InstallerCore.Report(progress,
                    "Conseil contextuel de Brest coop deja actif; aucune modification.");
                return;
            }
            ...
            InstallerCore.Log("Brest coop : conseil contextuel reactive.");
            InstallerCore.Report(progress,
                "Le conseil contextuel de Brest coop a ete reactive.");
        }
        ...
        private static byte[] PatchRegistry(byte[] data)
        {
            ...
            foreach (KeyValuePair<string, string> binding in bindings)
            {
                ...
                if (String.Equals(
                    ...
                    return data;
                throw new InvalidDataException(
                    "Le conseil contextuel de Brest coop possede deja un autre script : "
                    + binding.Value + ".");
            }
        ...
        private static List<KeyValuePair<string, string>> ReadBindings(byte[] data)
        {
            if (data == null || data.Length < 6)
                throw new InvalidDataException(
                    "Registre de scripts Brest coop trop court.");
            Encoding ansi = Encoding.GetEncoding(1252);
            ...
            byte[] data, ref int offset, Encoding encoding)
        {
            if (offset < 0 || offset > data.Length - 6
                ...
                throw new InvalidDataException(
                    "Champ invalide dans le registre Brest coop.");
            ...
            if (rawLength < 7 || rawLength > Int32.MaxValue
                ...
                throw new InvalidDataException(
                    "Taille de champ invalide dans le registre Brest coop.");
        ...
        private static void ValidateBinding(byte[] data)
        {
            ...
            if (found != 1)
                throw new InvalidDataException(
                    "La liaison du conseil contextuel dans Brest coop n'est pas unique.");
        }

        private static void ValidateAssets(string gamePath)
        {
            ValidateAsset(gamePath,
                "Scripts/Co_brest/hint.scr", ScriptArchives,
                new[] { "_PlayerInRange(2)", "HINT_Add(15600)" });
            ValidateAsset(gamePath,
                "Missions/Co_brest/scene2.bin", MissionArchives,
                new[] { "hint" });

            byte[] soloScript = ReadSource(ResolveSource(
                gamePath, "Scripts/Brest/hint.scr", ScriptArchives));
            byte[] coopScript = ReadSource(ResolveSource(
                gamePath, "Scripts/Co_brest/hint.scr", ScriptArchives));
            if (!BytesEqual(soloScript, coopScript))
                throw new InvalidDataException(
                    "Le conseil Brest coop differe de sa version solo active.");

            byte[] soloRegistry = ReadSource(ResolveSource(
                gamePath, "Missions/Brest/Scripts.dta", MissionArchives));
            int soloBindings = 0;
            foreach (KeyValuePair<string, string> binding
                in ReadBindings(soloRegistry))
                if (String.Equals(binding.Key, "hint",
                    StringComparison.OrdinalIgnoreCase)
                    && String.Equals(binding.Value, "hint.scr",
                    StringComparison.OrdinalIgnoreCase))
                    soloBindings++;
            if (soloBindings != 1)
                throw new InvalidDataException(
                    "La liaison de reference du conseil Brest solo est absente.");
        }
            ...
            string[] markers)
        {
            ...
            foreach (string marker in markers)
                if (text.IndexOf(marker, StringComparison.OrdinalIgnoreCase) < 0)
                    throw new InvalidDataException(
                        "Element Brest coop manquant dans " + relative + " : "
                        + marker + ".");
        }
