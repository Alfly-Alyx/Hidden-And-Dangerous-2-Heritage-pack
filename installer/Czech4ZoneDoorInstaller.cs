...
{
    internal static class Czech4ZoneDoorInstaller
    {
        private const string RegistryPath = "Missions/CZECH4_MP_ZONE/mpscripts.dta";
        private const string Actor = "door35";
        private const string Script = "door35.scr";
        ...
        public static string ValidateOnly(string gamePath)
        {
            ...
            if (!BytesEqual(patched, PatchRegistry(patched)))
                throw new InvalidDataException(
                    "La restauration de la porte Czech4 Zone n'est pas idempotente.");
            ValidateAssets(gamePath);
            return "Vestige Czech4 Zone verifie : door35 suit le meme script complet que les huit portes actives.";
        }
            ...
            Action<string> progress)
        {
            InstallerCore.Report(progress,
                "Reactivation de la porte door35 dans Czech4 Zone...");
            DormantSource source = ResolveSource(
                gamePath, RegistryPath, MissionArchives);
            string target = InstallerCore.SafeGameTarget(gamePath, RegistryPath);
            ...
            {
                InstallerCore.Report(progress,
                    "Porte door35 de Czech4 Zone deja active; aucune modification.");
                return;
            }
            ...
            InstallerCore.Log("Czech4 Zone : porte door35 reactivee.");
            InstallerCore.Report(progress,
                "La porte door35 de Czech4 Zone a ete reactivee.");
        }
        ...
        private static byte[] PatchRegistry(byte[] data)
        {
            ...
            foreach (KeyValuePair<string, string> binding in bindings)
            {
                ...
                if (String.Equals(
                    ...
                    return data;
                throw new InvalidDataException(
                    "La porte door35 possede deja un autre script : "
                    + binding.Value + ".");
            }
        ...
        private static List<KeyValuePair<string, string>> ReadBindings(byte[] data)
        {
            if (data == null || data.Length < 6)
                throw new InvalidDataException(
                    "Registre de scripts Czech4 Zone trop court.");
            Encoding ansi = Encoding.GetEncoding(1252);
            ...
            byte[] data, ref int offset, Encoding encoding)
        {
            if (offset < 0 || offset > data.Length - 6
                ...
                throw new InvalidDataException(
                    "Champ invalide dans le registre Czech4 Zone.");
            ...
            if (rawLength < 7 || rawLength > Int32.MaxValue
                ...
                throw new InvalidDataException(
                    "Taille de champ invalide dans le registre Czech4 Zone.");
        ...
        private static void ValidateBinding(byte[] data)
        {
            ...
            if (found != 1)
                throw new InvalidDataException(
                    "La liaison de door35 dans Czech4 Zone n'est pas unique.");
        }

        private static void ValidateAssets(string gamePath)
        {
            ValidateAsset(gamePath,
                "Scripts/CZECH4_MP_ZONE/door35.scr", ScriptArchives,
                new[] {
                    "OnUse()", "FRM_SetGroupLightMap(\"door35\",Door)",
                    "Door = 1"
                });
            ValidateAsset(gamePath,
                "Missions/CZECH4_MP_ZONE/scene2.bin", MissionArchives,
                new[] { "door35" });
            ValidateAsset(gamePath,
                "Missions/CZECH4_MP_ZONE/scene.4ds", MissionArchives,
                new[] { "door35" });
        }
            ...
            string[] markers)
        {
            ...
            foreach (string marker in markers)
                if (text.IndexOf(marker, StringComparison.OrdinalIgnoreCase) < 0)
                    throw new InvalidDataException(
                        "Element Czech4 Zone manquant dans " + relative + " : "
                        + marker + ".");
        }
